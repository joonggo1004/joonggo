...
public class ModifyProductHandler implements CommandHandler {
	...
	private String processSubmit(HttpServletRequest req, HttpServletResponse res) throws Exception {
		...
		String orgFileName = readService.getProduct(productNo, false).getFileName();
		
		Part filePart = req.getPart("file1");
		String fileName = filePart.getSubmittedFileName();
		//fileName = fileName == null ? orgFileName : fileName;
		if(fileName == null || fileName.isEmpty()) {
			fileName = orgFileName;
		}
		
		ModifyRequest modReq = new ModifyRequest(authUser.getId(), productNo, req.getParameter("title"), req.getParameter("content"), fileName);
		req.setAttribute("modReq", modReq);
		...
		if ((fileName != orgFileName) && !(fileName == null || fileName.isEmpty() || filePart.getSize() == 0)) {
			writeFile.write(filePart, productNo);
		}
		try {
